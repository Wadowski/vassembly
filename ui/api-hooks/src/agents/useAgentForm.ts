...
import { AgentCategory, type AgentFormValues, type UseAgentFormResult } from './types';
...
export const useAgentForm = (initial?: Partial<AgentFormValues>): UseAgentFormResult => {
  ...
  const blurField = useCallback(
    (key: keyof AgentFormValues) => {
      ...
      const fieldResult = fieldSchema.safeParse(values[key]);
      setFieldErrors((prev) => {
        const next = { ...prev };
        if (fieldResult.success) {
          delete next[key];
        } else {
          const first = fieldResult.error.issues[0];
