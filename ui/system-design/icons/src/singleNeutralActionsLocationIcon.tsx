import React from "react";
import { IconProps } from "./types";

export const SingleNeutralActionsLocationIcon = ({
  className,
  style,
}: IconProps) => (
  <svg
    style={style}
    className={className}
    xmlns="http://www.w3.org/2000/svg"
    fill="none"
    viewBox="0 0 24 24"
    strokeWidth="1.5"
  >
    <path
      stroke="currentColor"
      d="M18.75 16.187C18.5429 16.187 18.375 16.0191 18.375 15.812C18.375 15.6049 18.5429 15.437 18.75 15.437"
    ></path>
    <path
      stroke="currentColor"
      d="M18.75 16.187C18.9571 16.187 19.125 16.0191 19.125 15.812C19.125 15.6049 18.9571 15.437 18.75 15.437"
    ></path>
    <path
      stroke="currentColor"
      strokeLinecap="round"
      strokeLinejoin="round"
      d="M18.75 11.312C19.9435 11.312 21.0881 11.7861 21.932 12.63C22.7759 13.4739 23.25 14.6185 23.25 15.812C23.25 17.732 20.562 21.388 19.34 22.95C19.2699 23.0398 19.1802 23.1124 19.0778 23.1624C18.9754 23.2123 18.8629 23.2383 18.749 23.2383C18.6351 23.2383 18.5226 23.2123 18.4202 23.1624C18.3178 23.1124 18.2281 23.0398 18.158 22.95C16.937 21.389 14.25 17.733 14.25 15.813C14.2499 15.222 14.3662 14.6367 14.5923 14.0906C14.8183 13.5445 15.1498 13.0484 15.5677 12.6304C15.9855 12.2124 16.4817 11.8809 17.0277 11.6546C17.5737 11.4284 18.159 11.312 18.75 11.312V11.312Z"
    ></path>
    <path
      stroke="currentColor"
      strokeLinecap="round"
      strokeLinejoin="round"
      d="M6 6.75C7.65685 6.75 9 5.40685 9 3.75C9 2.09315 7.65685 0.75 6 0.75C4.34315 0.75 3 2.09315 3 3.75C3 5.40685 4.34315 6.75 6 6.75Z"
    ></path>
    <path
      stroke="currentColor"
      strokeLinecap="round"
      strokeLinejoin="round"
      d="M11.25 13.5C11.25 12.1076 10.6969 10.7723 9.71231 9.78769C8.72774 8.80312 7.39239 8.25 6 8.25C4.60761 8.25 3.27226 8.80312 2.28769 9.78769C1.30312 10.7723 0.75 12.1076 0.75 13.5V15.75H3L3.75 23.25H8.25L9 15.75H11.25V13.5Z"
    ></path>
  </svg>
);
