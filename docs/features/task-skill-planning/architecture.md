...
| **Output sections (Strategy map)** | `task`: Goal, Assumptions, Steps, Dependencies, Referenced skills. `scheduled_task`: + Scheduled time, Pre-execution prep, Reminders. `routine_task`: + Recurrence, Setup, Per-occurrence steps, Start/stop conditions. **Superseded for active `task` flow:** see [`subagent-orchestration/architecture.md`](../subagent-orchestration/architecture.md) — Task Planner now outputs a **subtask list** (one per specialization worker) instead of flat ordered steps; mandatory validator retry loop applies. **Superseded for plan persistence:** see [`task-plan/architecture.md`](../task-plan/architecture.md) — after composing the subtask list, Task Planner must call `persist_task_plan`; the plan is stored in `@vassembly/domain-task-plan-template` and `@vassembly/domain-task-plan-instance` and executed by the orchestration loop, not only returned in `agentResponse`. `skillIdsUsed` is now comment-scoped and derived during orchestration (Task Planner no longer calls `update_task` for skills). |
