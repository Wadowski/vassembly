...
Feature: Agent Internal Tools Assignment (Web UI)
  ...
  Scenario: Assign internal tools when creating an agent
    ...
    When I open the agent create form
    And I add internal tool "agent - use" to the agent form
    And I add internal tool "agent - list" to the agent form
  ...
  Scenario: Show assigned internal tools on agent edit form
    Given an agent exists for the current user with name "Assigned Tools Agent" assigned to internal tool id "agent-use"
    When I open the agent edit page
    Then I see internal tool "agent - use" assigned on the agent form

  Scenario: Remove internal tool from agent edit form
    Given a connected AI integration exists for the current user
    And an agent exists for the current user with name "Remove Tools Agent" assigned to internal tool ids "agent-use" and "agent-list"
    When I open the agent edit page
    And I remove internal tool "agent - list" from the agent form
    And I select the AI integration "E2E Web AI Credential"
    And I click "Update agent"
    Then I see the agent snackbar "Agent updated successfully"
    When I open the agent edit page
    Then I see internal tool "agent - use" assigned on the agent form
    And I do not see internal tool "agent - list" assigned on the agent form
